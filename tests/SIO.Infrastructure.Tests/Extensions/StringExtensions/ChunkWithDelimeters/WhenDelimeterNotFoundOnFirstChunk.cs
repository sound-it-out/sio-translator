...
{
    public class WhenDelimeterNotFoundOnFirstChunk : ChunkWithDelimetersSpecification
    {
        private const char _delimeter = '.';
        private const int _length = 120;
        ...
        [Then]
        public void FirstChunkShouldNotEndWithDelimeter()
        {
            Result.First().Should().NotEndWith(_delimeter.ToString());
        }

        [Then]
        public void FirstChunkShouldHaveExpectedLength()
        {
            Result.First().Should().HaveLength(_length);
        }
    }
}
